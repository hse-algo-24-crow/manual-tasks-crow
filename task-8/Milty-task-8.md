...
провидем повторную редукцию по строкам 
...
```mermaid
graph LR
    D(D - 0 фр.)  --> 3(3 - 1 фр.) -.-> E(E - 2 фр.) --> 5(5 - 1 фр.)
```
Построенное дерево содержит чередующуюся, относительно текущего паросочетания, цепь D3 - E5, цепь начинается и заканчивается в непокрытых вершинах, все ребра в цепи чередуются по вхождению в текущее паросочетание. Найденная цепь перекрашена.
...
graph LR
    ...
    E -.- 5
```
Есть непокрытая вершина C пробуем построить дерево.

```mermaid
graph LR
    C(C - 0 фр.)  --> 4(4 - 1 фр.) -.-> A(A - 2 фр.) --> 3(3 - 1 фр.) -.-> D(D- 0 фр.)
```
...
```mermaid
graph LR
    C(C - 0 фр.)  --> 4(4 - 1 фр.) -.-> A(A - 2 фр.) --> 3(3 - 1 фр.) -.-> D(D- 2 фр.)
...
```mermaid
graph LR
    C(C - 0 фр.)  --> 4(4 - 1 фр.) -.-> A(A - 2 фр.) --> 1(1 - 1 фр.) -.-> B(B- 0 фр.) --> 2(2 - 1 фр.)
```
Данное дерево содержит цепь, которая начинается и заканчивается в непокрытой вершине, внутри него ребра чередются. Цепь C4-A1-B2
