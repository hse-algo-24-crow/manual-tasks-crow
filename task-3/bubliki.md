...
* Из третьего уравнения выведем c<sub>n</sub> и подставим во второе, ведь нам нужно найти те маршруты, которые начинаются в А и заканчиваются B

$$
b_n = a_{n-1} + a_{n-2} + b_{n-2}
$$

* Также выведем a<sub>n</sub> из первого и подставим в полученное уравнение:

$$
b_n = b_{n-2} + c_{n-2} + a_{n-2} + b_{n-2} = 2*b_{n-2} + c_{n-2} + a_{n-2}
$$

Так как

$$
b_{n-1} = c_{n-2} + a_{n-2}
$$
